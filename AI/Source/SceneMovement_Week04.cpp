...
void SceneMovement_Week04::Init()
{
	...
	PostOffice::GetInstance()->Register("Scene", this); // <- "this" has an error whereby the Scene is NOT compatible with ObjectBase
...
void SceneMovement_Week04::Update(double dt)
{
	...
	for (std::vector<GameObject*>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
	{
		...
		else if (go->type == GameObject::GO_SHARK)
		{
			...
			{
				MessageWRU msgCheckFish = MessageWRU(go, MessageWRU::SEARCH_TYPE::SEARCH_NONE, 0.0f);
				Handle(&msgCheckFish);
			}
			else if (go->sm->GetCurrentState() == "Naughty")
			{
...
bool SceneMovement_Week04::Handle(Message* message)
{
	...
	if (messageWRU)
	{
		// get pointer to thte entity who fired the event
		...
		for (GameObject* go2 : m_goList)
		{
			...
			if (messageWRU->type == MessageWRU::NEAREST_FISHFOOD && go2->type == GameObject::GO_FISHFOOD)
			{
				float distance = (go->pos - go2->pos).Length();
			...
			{
				float distance = (go->pos - go2->pos).Length();
				if (distance < nearestDistance && (go->sm->GetCurrentState() == "TooFull" || go->sm->GetCurrentState() == "Full"))
				{
					nearestDistance = distance;
					go->nearest = go2;
				}
